...
export default function DiscoveryQuiz() {
  ...
  const [step, setStep] = useState(0); // 0=mood, 1=budget, 2=season, 3=results
  ...
  const results = useMemo(() => {
    ...
    if (filtered.length === 0) {
      filtered = getDestinations({ type: uniqueTypes.length > 0 ? uniqueTypes : undefined });
    }
    if (filtered.length === 0) {
      filtered = getDestinations();
    }
  ...
  const handleSelectResult = useCallback((dest) => {
    hideQuiz();
    // Small delay for the quiz exit animation
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-xl mx-4">
        ...
        <button
          onClick={hideQuiz}
          className="absolute -top-12 right-0 text-text-secondary hover:text-text-primary
        ...
        <motion.div
          className="bg-surface/95 backdrop-blur-xl border border-surface-raised rounded-card
            ...
            {[0, 1, 2, 3].map(i => (
              <div
                ...
                className={`h-1 flex-1 rounded-full transition-all duration-300
                  ${i <= step ? 'bg-accent-trail' : 'bg-surface-raised'}`}
              />
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            {step === 0 && (
              <motion.div key="mood" variants={containerVariants} initial="hidden" animate="visible" exit="exit">
                <h2 className="font-display text-2xl text-text-primary mb-2">
                  What's your <span className="text-accent-ochre">vibe</span>?
                </h2>
                    ...
                    <button
                      ...
                      className={`p-4 rounded-card border text-left transition-all duration-200
                        ${selectedMoods.includes(mood.id)
                          ? 'border-accent-trail bg-accent-trail/10 shadow-lg shadow-accent-trail/10'
                          : 'border-surface-raised hover:border-accent-trail/50'
                        }`}
                    >
                      <span className="text-2xl">{mood.icon}</span>
                      <div className="mt-2 font-body text-sm font-medium text-text-primary">
                        {mood.label}
                      </div>
                    </button>
                  ))}
                ...
                <button
                  ...
                  className="w-full mt-6 py-3 bg-accent-ochre text-bg-base rounded-card font-body
            ...
            {step === 1 && (
              <motion.div key="budget" variants={containerVariants} initial="hidden" animate="visible" exit="exit">
                <h2 className="font-display text-2xl text-text-primary mb-2">
                  What's the <span className="text-accent-ochre">budget</span>?
                </h2>
                ...
                <div className="space-y-3">
                  {BUDGETS.map(b => (
                    <button
                      ...
                      className={`w-full p-4 rounded-card border text-left transition-all duration-200
                        flex items-center gap-3
                        ${selectedBudget === b.id
                          ? 'border-accent-trail bg-accent-trail/10'
                          : 'border-surface-raised hover:border-accent-trail/50'
                        }`}
                    >
                      <span className="text-xl">{b.icon}</span>
                      <span className="font-body text-sm font-medium text-text-primary">{b.label}</span>
                    </button>
                  ))}
                </div>
                <div className="flex gap-3 mt-6">
                  <button onClick={prevStep} className="flex-1 py-3 border border-surface-raised text-text-secondary
                    rounded-card font-body text-sm hover:border-accent-trail/50 transition-colors">
                    ← Back
                  </button>
                  <button
                    onClick={nextStep}
                    className="flex-1 py-3 bg-accent-ochre text-bg-base rounded-card font-body
            ...
            {step === 2 && (
              <motion.div key="season" variants={containerVariants} initial="hidden" animate="visible" exit="exit">
                <h2 className="font-display text-2xl text-text-primary mb-2">
                  When are you <span className="text-accent-ochre">going</span>?
                </h2>
            ...
            {step === 3 && (
              <motion.div key="results" variants={containerVariants} initial="hidden" animate="visible" exit="exit">
                <h2 className="font-display text-2xl text-text-primary mb-2">
                  We found <span className="text-accent-ochre">{results.length}</span> match{results.length !== 1 ? 'es' : ''}
                </h2>
                  ...
                  <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                    {results.map((dest, idx) => (
                      <button
                        ...
                        className="w-full p-4 rounded-card border border-surface-raised text-left
                          hover:border-accent-trail hover:bg-surface-raised/50
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-display font-semibold text-text-primary
                                group-hover:text-accent-ochre transition-colors">
                                {dest.name}
                              </span>
                            </div>
                            ...
                            <div className="flex gap-1 mt-2">
                              {dest.type.map(t => (
                                <span key={t} className="text-[10px] bg-surface-raised text-accent-trail
                                  px-2 py-0.5 rounded-full font-mono uppercase">
                                  {t}
                                </span>
                              ))}
                            </div>
                          </div>
                          <span className="text-accent-trail group-hover:translate-x-1
                ...
                <button
                  onClick={() => { setStep(0); setSelectedMoods([]); setSelectedBudget(null); setSelectedSeasons([]); }}
                  className="w-full mt-4 py-3 border border-surface-raised text-text-secondary
                    rounded-card font-body text-sm hover:border-accent-trail/50 transition-colors"
                >
                  Start over
                </button>
